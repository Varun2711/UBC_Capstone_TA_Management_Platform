...
describe("AddOfferingModal", () => {
  ...
  describe("Rendering", () => {
    it("should render the modal with correct initial data", () => {
      ...
      expect(
        screen.getByRole("heading", { name: /add course offering/i })
      ).toBeInTheDocument();
      expect(
        screen.getAllByText(`${mockCourse.code} - ${mockCourse.title}`)
      ).toHaveLength(2);
  ...
  describe("Validation and Interaction", () => {
    ...
    it("should filter the professor list and allow selection", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          within(list).queryByText("Dr. Alan Turing")
        ).not.toBeInTheDocument();
      });
      ...
      expect(
        screen.getByRole("combobox", { name: /instructor/i })
      ).toHaveTextContent("Dr. Grace Hopper");
    ...
    it("should show an error for a duplicate section in the same term/year", async () => {
      ...
      expect(
        await screen.findByText(/section "section a" already exists/i)
      ).toBeInTheDocument();
    });
  });
