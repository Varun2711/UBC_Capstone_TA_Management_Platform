...
describe('AddCourseModal', () => {
  ...
  it('clears errors when user starts typing', async () => {
    ...
    expect(screen.queryByText('Course code is required')).not.toBeInTheDocument();
  });

  it('disables submit button while submitting', async () => {
    // Mock a slow response
    mockOnAddCourse.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 1000)));
    ...
    await userEvent.type(screen.getByLabelText('Course Title *'), 'Advanced Programming');
    await userEvent.type(screen.getByLabelText('Course Description *'), 'Advanced programming concepts and techniques');
  ...
  it('resets form when modal closes', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Course Title *'), 'Senior Project');

    // Verify the form has data
    expect(screen.getByLabelText('Course Code *')).toHaveValue('CS 401');
    expect(screen.getByLabelText('Course Title *')).toHaveValue('Senior Project');
