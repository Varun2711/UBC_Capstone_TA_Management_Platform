...
describe("EditOfferingModal", () => {
  ...
  const mockOfferingToEdit = {
    ...
    time_slots: [{ day: "Monday", time: "02:00 PM - 04:00 PM" }],
  };
  ...
  it("renders form with offering data", async () => {
    ...
    expect(
      screen.getByText(/CS 101 - Introduction to Programming/)
    ).toBeInTheDocument();
  ...
  it("validates section format", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Section must be 3 digits (e.g., 001, 002)")
      ).toBeInTheDocument();
    });
  });
  ...
  it("validates duplicate sections", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/section with this number already exists/)
      ).toBeInTheDocument();
    });
  });
