...
describe("ViewJobPostings", () => {
  ...
  it("shows 'View Application' button when student has already applied", async () => {
    const applications = [
      { posting_id: 1, application_id: 101 }, // student already applied to posting 1
    ];
    mockAxiosInstance.get
      .mockResolvedValueOnce({ data: mockJobPostings }) // job postings
      .mockResolvedValueOnce({ data: fakeProfile }) // profile
      .mockResolvedValueOnce({ data: applications }); // applications
  ...
  it("disables apply button and shows 'Application Closed' for closed jobs", async () => {
    const closedPosting = {
      ...mockJobPostings[0],
      status: "closed",
      deadline_date: "2023-01-01", // simulate past deadline
    };
    mockAxiosInstance.get
      .mockResolvedValueOnce({ data: [closedPosting] }) // job postings
      .mockResolvedValueOnce({ data: fakeProfile }) // profile
      .mockResolvedValueOnce({ data: [] }); // no applications
    ...
    expect(screen.getByText("Application Closed")).toBeDisabled();
  });
});
