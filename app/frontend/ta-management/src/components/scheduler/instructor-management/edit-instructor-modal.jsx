...
export function EditInstructorModal({ isOpen, onClose, onEditInstructor, instructor, existingInstructors = [], departments = [] }) {
  ...
  const validateField = (name, value) => {
    switch (name) {
      ...
      case "email":
        ...
        if (existingInstructors.find((i) => i.instructorId !== instructor?.instructorId && i.email.toLowerCase() === value.trim().toLowerCase())) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedInstructorData = await updateInstructor(instructor.instructorId, payload);
      
      onEditInstructor(updatedInstructorData);
      onClose();
  ...
  const hasChanges = instructor && (formData.name !== instructor.instructorName || formData.email !== instructor.email || formData.department !== instructor.department);

  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Edit className="h-5 w-5" />Edit Instructor</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-4">
          ...
          <div className="space-y-2">
            ...
            <div className="px-3 py-2 border rounded-md bg-muted/50 text-muted-foreground">{formData.employeeNumber}</div>
            <p className="text-xs text-muted-foreground">Employee number cannot be changed.</p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="edit-email">Email Address *</Label>
            <Input id="edit-email" type="email" value={formData.email} onChange={(e) => handleInputChange("email", e.target.value)} onBlur={(e) => handleBlur("email", e.target.value)} className={errors.email ? "border-red-500" : ""} />
