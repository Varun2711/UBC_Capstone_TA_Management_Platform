...
import { Bell, Plus, Calendar, Upload } from "lucide-react";
...
export default function AdminCourseManagement() {
  ...
  const [isAddLabTutorialModalOpen, setIsAddLabTutorialModalOpen] =
    useState(false);
  const [isEditSharedSessionModalOpen, setIsEditSharedSessionModalOpen] =
    useState(false);
  ...
  const handleDeleteOffering = async (offering) => {
    if (
      window.confirm(
        `Are you sure you want to delete Section ${offering.section}? This action cannot be undone.`
      )
    ) {
      try {
  ...
  const handleAddLabTutorialSubmit = async (
    courseId,
    term,
    year,
    sessionType,
    ...
    try {
      ...
      const selectedTerm = terms.find(
        (t) => t.value === term && t.year.toString() === year
      );
    ...
    const foundCourse = courses.find((course) => {
      if (course.sharedSessions && course.sharedSessions[termKey]) {
        ...
        if (sessionsInTerm[sessionTypePlural]) {
          return sessionsInTerm[sessionTypePlural].some(
            (s) => s.id === session.id
          );
        }
      }
  ...
  const handleEditSharedSessionSubmit = async (
    sessionId,
    updatedSessionData
  ) => {
    try {
  ...
  const handleBulkImport = async () => {
    ...
    setIsImporting(true);
    try {
      const result = await bulkImportCourses(selectedFile);
      ...
      const totalCreated =
        (result.results?.courses_created || 0) +
        (result.results?.courses_updated || 0) +
        (result.results?.course_offerings_created || 0) +
        (result.results?.shared_sessions_created || 0);

      // Show detailed success message
      const details = [];
      if (result.results?.courses_created > 0) {
        details.push(`${result.results.courses_created} courses created`);
      }
      if (result.results?.courses_updated > 0) {
      ...
      const errorMessage =
        ...
        error.message ||
        "Failed to import courses. Please check your CSV file format.";
  ...
  const filteredCourses = courses
    .filter((course) => {
      ...
      const matchesYear =
        ...
        (() => {
          ...
          const hasOfferingForYear =
            course.offerings?.some(
              (offering) => String(offering.year) === String(selectedYear)
            ) || false;

          // Check shared sessions for the year
          const hasSharedSessionForYear =
            Object.keys(course.sharedSessions || {}).some((termKey) => {
      ...
      const matchesTerm =
        ...
        (() => {
          ...
          const hasOfferingForTerm =
            course.offerings?.some((offering) => {
              const formattedOfferingTerm = formatTermForComparison(
                offering.term
              );
      ...
      const finalResult =
        matchesSearch && matchesDepartment && matchesYear && matchesTerm;
    ...
    .map((course) => {
      ...
      if (selectedYear !== "all" || selectedTerm !== "all") {
        filteredCourse.offerings =
          course.offerings?.filter((offering) => {
            const matchesYear =
              selectedYear === "all" ||
  ...
  const years = (() => {
    ...
    const finalYears = Array.from(yearSet).sort(
        ...
        <main className="flex-1 space-y-6 p-4 md:p-8">
          ...
          <div className="space-y-4">
            {filteredCourses.map((course) => {
              ...
              const visibleOfferingsCount = course.offerings.filter(
                (offering) => {
                  const matchesYear =
                    selectedYear === "all" ||
                    String(offering.year) === String(selectedYear);
      ...
      >
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-blue-700">
                    Download our sample CSV to see the required format and
                    column headers.
                  </p>
                </div>
                <Button
            ...
            <label
              htmlFor="csv-upload"
              className="block border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-gray-400 hover:bg-gray-50 transition-colors"
            >
